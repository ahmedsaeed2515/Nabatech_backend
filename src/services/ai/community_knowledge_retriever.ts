...
export type CommunityContextResult = {
  text: string;
  hasData: boolean;
  meta: {
    similarCasesCount: number;
    confirmedCount: number;
    recentOutbreaksCount: number;
 ...
 */
export const retrieveCommunityContext = async (
  diseaseNameEn?: string,
  question?: string
): Promise<CommunityContextResult> => {
  ...
  try {
    ...
    if (matchedDiseaseName) {
      meta.similarCasesCount = await DiagnosisHistory.countDocuments({
        diseaseNameEn: { $regex: new RegExp(`^${matchedDiseaseName}$`, "i") },
        feedbackStatus: { $ne: "rejected" }
      });
      meta.confirmedCount = await DiagnosisHistory.countDocuments({
        diseaseNameEn: { $regex: new RegExp(`^${matchedDiseaseName}$`, "i") },
        feedbackStatus: "confirmed"
      });
      
      const fourteenDaysAgo = new Date(Date.now() - 14 * 24 * 60 * 60 * 1000);
      meta.recentOutbreaksCount = await DiagnosisHistory.countDocuments({
        diseaseNameEn: { $regex: new RegExp(`^${matchedDiseaseName}$`, "i") },
    ...
    let matchingHistories: any[] = [];
    if (matchedDiseaseName) {
      matchingHistories = await DiagnosisHistory.find({
        diseaseNameEn: { $regex: new RegExp(`^${matchedDiseaseName}$`, "i") }
      }).select("_id").lean();
    }
    ...
    if (postIds.length > 0) {
      ...
      acceptedOffers = await SpecialistOffer.find({
        post: { $in: postIds },
        status: "accepted"
      })
      .limit(3)
    ...
    const expertComments = comments.filter(
      c => c.author && (c.author as any).role === UserRole.EXPERT
    );
