...
export class AiRecommendationService {
   ...
   */
  async generateRecommendations(userId: string, weatherData: any) {
    try {
      // 1. Gather User Context in parallel
      const [user, plants, diagnoses] = await Promise.all([
        User.findById(userId),
        PlantModel.find({ user: userId }),
        DiagnosisHistory.find({ user: userId }).sort({ createdAt: -1 }).limit(3)
      ]);
      ...
      const placements = await ExplorePlacement.find({ isActive: true });
      const articles = await Article.find({ isPublished: true }).limit(15);
...
{"recommendations": [{"id": "item_id", "reason": "A clear explanation why this is recommended based on the user's weather, plant growth stage, or disease history."}]}`;
      ...
      logger.error('Error in AiRecommendationService:', error);
      // Fallback: return first 5 active placements or articles
      const fallbackList = [];
      const activePlacements = await ExplorePlacement.find({ isActive: true }).limit(5);
