import { Request, Response } from "express";
import CommunityPoll from "../models/community_poll_model";
import CommunityPollOption from "../models/community_poll_option_model";
import CommunityPollVote from "../models/community_poll_vote_model";
import { logger } from "../utils/logger";

// @desc    Vote on a poll
// @route   POST /api/community/polls/:pollId/vote
// @access  Private
export const voteOnPoll = async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
    const pollId = req.params.pollId;
    const { optionId } = req.body;

    if (!optionId) {
      return res.status(400).json({ error: "Option ID is required", errorCode: "VALIDATION_FAILED" });
    }

    const poll = await CommunityPoll.findById(pollId);
    if (!poll) {
      return res.status(404).json({ error: "Poll not found", errorCode: "RESOURCE_NOT_FOUND" });
    }

    const option = await CommunityPollOption.findOne({ _id: optionId, poll: pollId });
    if (!option) {
      return res.status(404).json({ error: "Poll option not found", errorCode: "RESOURCE_NOT_FOUND" });
    }

    const existingVote = await CommunityPollVote.findOne({ poll: pollId, user: userId });
    if (existingVote) {
      return res.status(400).json({ error: "You have already voted on this poll", errorCode: "ALREADY_VOTED" });
    }

    // Record vote
    await CommunityPollVote.create({
      poll: pollId,
      option: optionId,
      user: userId,
    });

    // Increment counts
    await CommunityPollOption.updateOne({ _id: optionId }, { $inc: { votes: 1 } });
    await CommunityPoll.updateOne({ _id: pollId }, { $inc: { totalVotes: 1 } });

    // Fetch updated poll
    const updatedPoll = await CommunityPoll.findById(pollId);
    const options = await CommunityPollOption.find({ poll: pollId }).sort({ sortOrder: 1 });

    res.status(200).json({
      data: {
        poll: {
          id: updatedPoll!._id.toString(),
          question: updatedPoll!.question,
          options: options.map(o => ({
            id: o._id.toString(),
            text: o.text,
            votes: o.votes
          })),
          userVotedOptionId: optionId,
          totalVotes: updatedPoll!.totalVotes
        }
      }
    });

  } catch (error: any) {
    if (error.code === 11000) {
      return res.status(400).json({ error: "You have already voted on this poll", errorCode: "ALREADY_VOTED" });
    }
    logger.error('Failed to vote on poll', { error });
    res.status(500).json({ message: "Failed to vote on poll" });
  }
};


