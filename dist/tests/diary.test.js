...
describe("Diary API Endpoints", () => {
    ...
    describe("Authorization Security Checks", () => {
        it("should reject GET /api/diary if no token is provided", async () => {
            const res = await (0, supertest_1.default)(app_1.default).get("/api/diary");
        ...
        it("should reject PUT /api/diary/:id if no token is provided", async () => {
            const res = await (0, supertest_1.default)(app_1.default).put("/api/diary/650c1f2e9f1a2c3d4e5f6a7b");
            expect(res.status).toBe(401);
        });
        it("should reject DELETE /api/diary/:id if no token is provided", async () => {
            const res = await (0, supertest_1.default)(app_1.default).delete("/api/diary/650c1f2e9f1a2c3d4e5f6a7b");
            expect(res.status).toBe(401);
        });
    });
    describe("POST /api/diary", () => {
        ...
        it("should successfully create a new diary entry for authenticated user", async () => {
            ...
            const dbEntry = await diary_entry_model_1.default.findById(res.body.entry.id);
    ...
    describe("PUT /api/diary/:id", () => {
        ...
        beforeEach(async () => {
            ...
            const entryRes = await (0, supertest_1.default)(app_1.default)
                .post("/api/diary")
                .set("Authorization", `Bearer ${userToken}`)
                .send({
                plantId: "650c1f2e9f1a2c3d4e5f6a7b",
        ...
        it("should successfully update diary entry owned by the user", async () => {
            ...
            // Verify db changes
            const dbEntry = await diary_entry_model_1.default.findById(entryId);
