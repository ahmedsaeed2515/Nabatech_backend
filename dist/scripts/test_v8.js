...
async function testV8() {
    ...
    try {
        ...
        const res = await axios_1.default.get(`${BASE_URL}/health`, { timeout: 30000 });
    ...
    try {
        ...
        const start = Date.now();
        const res = await axios_1.default.get(`${BASE_URL}/health/deep`, { timeout: 60000 });
        console.log(`   ✅ Success (${Date.now() - start}ms):`, JSON.stringify(res.data));
    }
    catch (e) {
        console.log(`   ❌ Failed: ${e.message}`);
    ...
    try {
        console.log("\n3. Testing GET /stats ...");
        const start = Date.now();
        const res = await axios_1.default.get(`${BASE_URL}/stats`, { timeout: 30000 });
        console.log(`   ✅ Success (${Date.now() - start}ms):`, JSON.stringify(res.data));
    }
    catch (e) {
        console.log(`   ❌ Failed: ${e.message}`);
    ...
    try {
        ...
        const res = await axios_1.default.post(`${BASE_URL}/retrieve`, payload, { timeout: 45000 });
    ...
    try {
        ...
        console.log(`   Sending payload:`, payload);
        const res = await axios_1.default.post(`${BASE_URL}/ask`, payload, { timeout: 60000 });
        console.log(`   ✅ Success (${Date.now() - start}ms):`);
        const data = res.data;
