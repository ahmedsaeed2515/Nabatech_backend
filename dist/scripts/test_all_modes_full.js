...
async function runTests() {
    ...
    try {
        ...
        const ragResult = await (0, rag_provider_1.retrieveRagChunks)(settings, "Tomato_Early_Blight", question, 5, "ar");
        ...
        const llmResult = await (0, llm_provider_1.askLlm)(settings, prompt, "llm", [], "chat");
    ...
    for (const m of hfModes) {
        ...
        try {
            const start = Date.now();
            const res = await (0, hf_integrated_provider_1.askHuggingFaceIntegrated)(m.id, m.url, question, [], 60000);
            const ms = Date.now() - start;
            if (res.success) {
                console.log(`✅ SUCCESS (${ms}ms)`);
