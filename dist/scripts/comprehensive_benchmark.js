...
const community_knowledge_retriever_1 = require("../services/ai/community_knowledge_retriever");
...
async function benchmarkProvider(name, model, endpoint, type, apiKey, timeoutMs, testType, promptText, supportsStreaming) {
...
function calculateStats(arr) {
    if (arr.length === 0)
        return { avg: 0, min: 0, max: 0, median: 0, stdDev: 0, successRate: 0 };
...
async function runE2EStageTrace() {
    ...
    if (settings.cnn.enabled && settings.cnn.endpointUrl) {
        ...
        try {
            await (0, cnn_provider_1.runCnnDiagnosis)(settings, formData, formData.getHeaders());
        }
        catch (e) { }
    ...
    if (settings.rag.enabled && settings.rag.endpointUrl) {
        ...
        try {
            await (0, rag_provider_1.retrieveRagChunks)(settings, prediction, "How to treat this?", 4, "en");
        }
        catch (e) { }
    ...
    try {
        await (0, community_knowledge_retriever_1.retrieveCommunityContext)(prediction, "How to treat this?");
    }
    catch (e) { }
    trace["Community Retrieval"] = performance.now() - commStart;
    // Prompt Construction
    const promptStart = performance.now();
    (0, assistant_prompt_builder_1.buildAssistantPrompt)({
...
async function run() {
    ...
    const settings = await (0, ai_config_service_1.getAiSettings)();
    const dbProviders = await ai_provider_settings_model_1.default.find({ enabled: true });
    const allProviders = [];
    dbProviders.forEach(p => {
        let type = "generic_llm";
        const nameStr = p.providerName.toLowerCase();
        const urlStr = p.baseUrl.toLowerCase();
        if (nameStr.includes("gemini") || urlStr.includes("generativelanguage"))
            type = "gemini";
        else if (nameStr.includes("anthropic"))
            type = "anthropic";
        else if (nameStr.includes("cohere"))
            type = "cohere";
        else if (urlStr.includes("hf.space") || nameStr.includes("huggingface"))
            type = "huggingface_inference";
        else if (nameStr.includes("openai") || urlStr.includes("openai"))
            type = "openai_compatible";
        if (nameStr.includes("agentrouter"))
            return;
        allProviders.push({
            name: p.providerName,
            model: p.llmModel,
            endpoint: p.baseUrl,
            type,
            apiKey: (0, secret_crypto_1.decryptSecret)(p.apiKeyEncrypted),
            timeoutMs: 30000,
    ...
    for (const p of allProviders) {
        if (p.type === "hf_custom") {
            ...
            try {
                await (0, hf_integrated_provider_1.askHuggingFaceIntegrated)(p.name, p.endpoint, "OK", [], p.timeoutMs);
            }
            catch (e) { }
            const res = {
            ...
            for (let i = 0; i < TEST_ITERATIONS; i++) {
                const start = performance.now();
                const output = await (0, hf_integrated_provider_1.askHuggingFaceIntegrated)(p.name, p.endpoint, "What are the common diseases of tomatoes?", [], p.timeoutMs);
    ...
    fs_1.default.writeFileSync(path_1.default.join(__dirname, "../../../BENCHMARK_RESULTS.md"), md);
