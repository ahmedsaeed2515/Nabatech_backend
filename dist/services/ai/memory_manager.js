...
class MemoryManager {
    static async saveShortTermMemory(userId, key, value, ttlMinutes = 60) {
        ...
        const existing = await ai_memory_model_1.default.findOne({ userId, type: "short_term", key });
        if (existing) {
    ...
    static async getShortTermMemory(userId, key) {
        const record = await ai_memory_model_1.default.findOne({ userId, type: "short_term", key });
    ...
    static async getLongTermMemory(userId, key) {
        const record = await ai_memory_model_1.default.findOne({ userId, type: "long_term", key });
        return record?.value;
    }
    static async getAllContext(userId) {
        const records = await ai_memory_model_1.default.find({ userId });
        const now = new Date();
        const context = {
            shortTerm: {},
            longTerm: {}
