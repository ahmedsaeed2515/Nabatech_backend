...
const makeProvider = (name, taskRole, enabled = true) => ({
    name,
    enabled,
    taskRole,
...
describe("llm_provider — taskRole filtering", () => {
    ...
    it("includes 'search' provider when taskRole is 'search'", async () => {
        ...
        const result = await (0, llm_provider_1.askLlm)(settings, "test query", "llm", [], "search");
    ...
    it("excludes 'search' provider when taskRole is 'chat'", async () => {
        ...
        const result = await (0, llm_provider_1.askLlm)(settings, "test question", "llm", [], "chat");
    ...
    it("excludes 'chat' provider when taskRole is 'search'", async () => {
        ...
        await expect((0, llm_provider_1.askLlm)(settings, "test query", "llm", [], "search")).rejects.toThrow();
    });
    // ── Test 4 ──────────────────────────────────────────────────
    ...
    it("HF fallback is NOT triggered when taskRole is 'search' and providers fail", async () => {
        ...
        const hfCalls = mockedAxios.post.mock.calls.filter((c) => String(c[0]).includes("hf") || String(c[0]).includes("ask"));
